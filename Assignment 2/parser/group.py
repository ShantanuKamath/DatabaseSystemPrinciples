...
def group(plan, start=False):
    ...
    parsed_plan += " " + parser.get_conjuction()
	group_keys = ",".join([k.replace("::text", "") for k in plan["Group Key"][:-1]] 
	plurality = "s" if len(plan["Plans"]) > 0 else ""
	parsed_plan += "the result from the previous operation is grouped together using the key{} ".format(plurality)

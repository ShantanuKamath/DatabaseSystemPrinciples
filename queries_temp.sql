...
FROM PublicationAuthorTemp AP JOIN PublicationTemp P ON AP.publication_id = P.publication_id
...
\echo Query 3c
SELECT A.name
FROM PublicationAuthorTemp AP JOIN AuthorTemp A ON AP.author_id = A.author_id
WHERE AP.publication_id IN (
	SELECT P.publication_id
	FROM PublicationTemp P
...
FROM (SELECT * FROM AuthorTemp) as q1, (
	SELECT PA1.author_id AS author_id,COUNT(DISTINCT PA2.author_id) AS collaborators_count
	FROM PublicationAuthorTemp PA1, PublicationAuthorTemp PA2
...
AND q1.author_id IN (
	SELECT AP.author_id
	FROM PublicationAuthorTemp AP JOIN PublicationTemp P
	ON P.publication_id = AP.publication_id
...
FROM AuthorTemp A, PublicationTemp P, PublicationAuthorTemp PA
...
--------------------------------------------------------------------------------
--------------test_version------------------------
-- SELECT conf_name AS conf_name, year, COUNT(*) AS count
\echo Query 8
SELECT DISTINCT conf_name
FROM PublicationTemp
WHERE mdate LIKE '%-07-%'
...
FROM AuthorTemp A JOIN PublicationAuthorTemp AP ON A.author_id = AP.author_id
JOIN PublicationTemp P ON AP.publication_id = P.publication_id
...
\echo Query 9b
SELECT A.author_id, A.name, COUNT(*)
FROM AuthorTemp A JOIN PublicationAuthorTemp AP
